...
function enviarCorreoAprobacion(
  ...
  // URL del Web App
  const webAppUrl = "https://script.google.com/macros/s/AKfycbz1JSWod6VULDlB7MhjdzUS-gYi9eJnni3GrFZHxakjDt2tCAEub3dZvV25N6RdzYiS/exec"; // Reemplaza con tu URL de Web App
...
function doPost(respuesta_solicitud) {
  ...
  var comentario = respuesta_solicitud.parameter.comentario || "";
  ...
  for (let i = 0; i < valoresColumnaL.length; i++) {
    if (valoresColumnaL[i][0] === solicitudId) {
  ...
  if (filaEncontrada !== -1) {
    ...
    var celdaComentario = hojaRespuestas.getRange(filaEncontrada, 14); // Columna N
    var respuestaExistente = celdaRespuesta.getValue();
    var correoRespuestasolicitante = hojaRespuestas.getRange(filaEncontrada, 2).getValue();
...
function enviarCorreoRespuesta(solicitudId, accion, comentario, correoRespuestasolicitante) { 
...
// Codigo funcional del formulario y la respuesta